...
- **Works everywhere a pager goes** — **a vertical side panel behaves identically to a
  ...
  **scale to fit** on both axes rather than overflowing onto neighbours. Optional **Appearance →
  Vertical panels → "Match the virtual-desktop grid layout"** lays a multi-row grid out in KWin's
...
| `matchDesktopGrid` | `false` | **Vertical panels only:** lay a multi-row grid out in KWin's orientation (rows top-to-bottom, like the stock pager) instead of transposing it down the panel. No effect on a horizontal panel. |
